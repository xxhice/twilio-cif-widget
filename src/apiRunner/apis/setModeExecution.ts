...
export class setModeExecution implements IApiExecution {
  async execute(context: IApiExecutionContext): Promise<IApiExecutionResult> {
    try {
      const modeToSet = 1; // Always set expanded mode
      await CIFV2.getInstance().setMode(modeToSet, context.correlationId);
      return {
