...
const TwilioAgentPanel: React.FC = () => {
  ...
  // Tracks conversation SIDs for which onConversationLoad has already been raised.
  // Per CIF/HIA contract, onConversationLoad fires once per conversation lifecycle
  // (initial accept), not on every focus switch. Cleared when the session closes.
  ...
  const loadConversation = React.useCallback(async (conv: Conversation) => {
    try {
      ...
      // Per the CIF/HIA contract, onConversationLoad fires once per conversation lifecycle
      // (initial accept), not on every focus switch. Gating on loadedConversationsRef
      // prevents repeated raises when the agent refocuses on an already-loaded session.
      if (!loadedConversationsRef.current.has(conv.sid)) {
        ...
        console.log('[HIA] onConversationLoad already raised for this conversation, skipping', { conversationId: conv.sid });
      }
          ...
          CIFV2.getInstance().addHandler('onSessionClosed', async (eventData: string) => {
            try {
              ...
              if (conversationSid) {
                ...
                // Allow onConversationLoad to fire again if this conversation is re-accepted later
                loadedConversationsRef.current.delete(conversationSid);
        ...
        twilioClient.on('conversationJoined', async (conv: Conversation) => {
          ...
          try {
            ...
            if (result.actionName === 'Accept') {
              ...
              // Create a new CIF session - reuse the notify correlationId so the
              // notify and createSession calls are grouped in CIF telemetry.
  ...
  const handleJoinConversation = async (conversationSid: string) => {
    ...
    try {
      ...
      const sessionId = await CIFV2.getInstance().createSession(
        "msdyn_3p_session", // Session template name
        Utils.generateUUID(), // Correlation ID
        {
